...
impl<'a> PuzzleImageBuilder<'a> {
    ...
    pub fn build(self) -> PuzzleImage<'a> {
        ...
        let changed_cells = if let Some(cell_changes) = self.cell_changes {
            cell_changes.iter().map(|(&i, _)| i).collect()
        } else {
            Vec::new()
        };
        PuzzleImage {
...
impl PuzzleImage<'_> {
    pub fn save_svg(&self, path: &Path) -> io::Result<()> {
        ...
        write!(writer, "{}", PuzzleSvg(self))
    }
}

struct PuzzleSvg<'a>(&'a PuzzleImage<'a>);

impl Display for PuzzleSvg<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        PuzzleSvgContext::new(self.0, f).write()
    }
}

struct PuzzleSvgContext<'a, W: Write> {
    image: &'a PuzzleImage<'a>,
    xml: Xml<'a, W>,
}

impl<'a, W> PuzzleSvgContext<'a, W>
where
    W: Write,
{
    ...
    fn cages_outline(&mut self) -> Result {
        ...
        let mut between_cells = |a: Coord<usize>, b: Coord<usize>| {
            if puzzle.cell(a).cage_id() != puzzle.cell(b).cage_id() {
                write!(
                    d,
                    "M{}{}{}",
                    path_coord(cell_coord(b)),
                    if a.row() == b.row() { 'v' } else { 'h' },
                    CELL_WIDTH
                )
                .unwrap();
            }
