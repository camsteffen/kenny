...
impl<'a> AsImage for Solver<'a> {
    fn as_image(&self) -> RgbImage {
        let image = self.puzzle.as_image();
        draw_markup(image, info, self);
        image
    }
}
    ...
    solver: &Solver)
{
    for (pos, cell) in solver.cells.iter_coord() {
        match *cell {
            Variable::Unsolved(ref domain) => {
                draw_cell_domain(image, info, pos, domain)
            },
            Variable::Solved(value) => {
                draw_cell_solution(image, info, pos, value)
            },
        };
    }
}
    ...
    value: i32)
{
    ...
    let point = point(x, y);
    println!("pos {} point {} {}, a {}, b {}", pos, x, y, info.cell_width - info.border_width, v_metrics.ascent);
